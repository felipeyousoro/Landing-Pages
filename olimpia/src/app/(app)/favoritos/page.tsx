...
export default function FavoritosPage() {
  ...
  const favorites = useMemo(
    () => conversations.filter((c) => c.favorite),
    [conversations],
  );
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Conversas marcadas para acesso rápido.
          </p>
        </div>
        <Badge tone="warning">{favorites.length} favoritos</Badge>
      </div>
      ...
      {favorites.length === 0 ? (
        <Card className="hover:shadow-[var(--shadow-sm)]">
          <EmptyState
            ...
            description="Marque conversas com a estrela no histórico ou no chat para encontrá-las aqui."
          />
        </Card>
      ) : filtered.length === 0 ? (
        <Card className="hover:shadow-[var(--shadow-sm)]">
